...
import {
  ...
  enrichTopicFunnel,
  suggestMapCta,
} from "../../src/lib/content/funnel.ts";
...
import type { Article, Topic } from "../../src/lib/content/schema.ts";
import { INTENT_META, type AffiliateIntent } from "../../src/lib/affiliate/intents.ts";
import { argValue, slugify } from "./_shared.mts";
...
async function loadTopic(): Promise<Topic> {
  ...
  const scored = topics.filter((t) => t.status === "scored" || t.status === "new");
...
> {
  ...
  // Ensure ≥2 source slots for checklist (duplicate labeled if only one)
  if (sources.length === 1) {
    sources.push({
  ...
  const clipHook =
    funnel.clipHook || buildClipHook(topic.headline, funnel.funnelKind ?? "mixed");
...
- Affiliate product slots fill as official ASINs go live — we prioritize purchase intent over rumor recaps.
`
      : "";
...
- **Rumors**: community leaks stay labeled as unverified — we will not invent a trailer date.
...
`;
  ...
  return {
    ...
    locale: "en",
    title,
    description: description.length < 120 ? description.padEnd(120, ".") : description,
    bodyMarkdown,
...
Rules: never invent trailer dates; label rumors; include ≥3 markdown internal links to /en/map, /en/locations, /en/guides/*;
...
async function main() {
  ...
  console.log(`\nDraft saved id=${article.id} slug=${article.slug}`);
  console.log(`SEO score=${checklist.score}`);
  if (checklist.issues.length) {
    console.log("Issues:");
    for (const i of checklist.issues) console.log(`  - ${i}`);
  }
