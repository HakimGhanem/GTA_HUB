...
import { KEYWORDS, getP0Keywords } from "../../src/data/keywords.ts";
import { enrichTopicFunnel } from "../../src/lib/content/funnel.ts";
import type { ContentCluster } from "../../src/lib/content/schema.ts";
import { getTopicByEventKey, upsertTopic } from "../../src/lib/content/repository.ts";
import { eventKeyFromHeadline } from "./_shared.mts";
...
const FEEDS = [
  ...
  // Purchase / setup intent — higher EPC near launch
  ...
  // FR trends (local affiliate store)
  "https://news.google.com/rss/search?q=GTA+6+OR+%22GTA+VI%22+pr%C3%A9commande&hl=fr&gl=FR&ceid=FR:fr",
];

const DOMAIN_BOOST = ["rockstargames.com", "ign.com", "bloomberg.com", "gamespot.com", "amazon."];
...
function inferCluster(text: string): ContentCluster {
  ...
  if (t.includes("headset") || t.includes("ssd") || t.includes("120hz") || t.includes("setup"))
    return "setup";
  if (t.includes("tiktok") || t.includes("clip") || t.includes("stream")) return "clip";
  if (t.includes("trailer") || t.includes("gameplay")) return "trailer";
  if (t.includes("pre-order") || t.includes("preorder") || t.includes("collector") || t.includes("précommande"))
    return "preorder";
  if (t.includes("release date") || t.includes("launch") || t.includes("delay"))
    return "release";
  if (t.includes("collectible") || t.includes("hidden package")) return "collectibles";
  if (t.includes("vice city") || t.includes("location") || t.includes("leonida"))
    return "locations";
...
function scoreItem(title: string, link: string, pubDate?: string): number {
  ...
  for (const kw of getP0Keywords()) {
    if (lower.includes(kw.phrase.replace(/gta 6 /g, "").slice(0, 12))) score += 15;
...
function keywordHint(cluster: ContentCluster): string | undefined {
  return KEYWORDS.find((k) => k.cluster === cluster && k.priority === "P0")?.phrase;
}
...
async function main() {
  ...
  for (const item of parserItems) {
    ...
    created++;
    console.log(
      `+ [f${enriched.funnelScore}] ${enriched.funnelKind}/${cluster} — ${title.slice(0, 70)}`,
    );
  }
