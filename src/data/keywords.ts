import type { KeywordRecord } from "@/lib/content/schema";

/**
 * Revenue-oriented keyword strategy for Map-6.
 * 1 article = 1 primary keyword + 2–4 secondaries from the same cluster.
 */
export const KEYWORDS: KeywordRecord[] = [
  // ── Trailer (timely news) ──────────────────────────────────────
  {
    id: "kw-trailer-3",
    phrase: "gta 6 trailer 3",
    cluster: "trailer",
    priority: "P0",
    locale: "en",
    targetSlugHint: "gta-6-trailer-3-what-we-know",
    relatedInternalPaths: [
      "/map",
      "/guides/gta-6-extended-look-how-to-watch",
      "/guides/gta-6-map-guide",
    ],
    competitorNotes: "Extended Look is the dated official drop — not a rumor calendar",
    status: "published",
    monetization: "mixed",
  },
  {
    id: "kw-trailer-date",
    phrase: "gta 6 trailer date",
    cluster: "trailer",
    priority: "P0",
    locale: "en",
    targetSlugHint: "gta-6-trailer-3-what-we-know",
    relatedInternalPaths: [
      "/guides/gta-6-trailer-3-what-we-know",
      "/map",
      "/guides/gta-6-preorder-guide",
    ],
    status: "published",
    monetization: "adsense",
  },
  {
    id: "kw-extended-look",
    phrase: "gta 6 extended look",
    cluster: "trailer",
    priority: "P0",
    locale: "en",
    targetSlugHint: "gta-6-extended-look-how-to-watch",
    relatedInternalPaths: [
      "/guides/gta-6-extended-look-how-to-watch",
      "/news/gta-6-extended-look-watch-times",
      "/map",
    ],
    competitorNotes: "Official 27 Aug 2026 drop — Netflix first, YouTube +6h; no fake runtime",
    status: "published",
    monetization: "mixed",
  },
  {
    id: "kw-netflix-trailer",
    phrase: "gta 6 netflix trailer",
    cluster: "trailer",
    priority: "P0",
    locale: "en",
    targetSlugHint: "gta-6-extended-look-watch-times",
    relatedInternalPaths: [
      "/guides/gta-6-extended-look-how-to-watch",
      "/news/gta-6-extended-look-watch-times",
      "/creators",
    ],
    status: "published",
    monetization: "adsense",
  },
  {
    id: "kw-map-watch-for",
    phrase: "gta 6 map trailer",
    cluster: "map",
    priority: "P0",
    locale: "en",
    targetSlugHint: "gta-6-extended-look-map-watch-for",
    relatedInternalPaths: [
      "/news/gta-6-extended-look-map-watch-for",
      "/map",
      "/news/gta-6-extended-look-live-notes",
    ],
    competitorNotes: "Pause list for official Extended Look — no invented POIs",
    status: "published",
    monetization: "map",
  },
  {
    id: "kw-live-notes",
    phrase: "gta 6 extended look live",
    cluster: "trailer",
    priority: "P0",
    locale: "en",
    targetSlugHint: "gta-6-extended-look-live-notes",
    relatedInternalPaths: [
      "/news/gta-6-extended-look-live-notes",
      "/news/gta-6-extended-look-map-watch-for",
      "/map",
    ],
    competitorNotes: "Live desk updates after 21:00 CEST from official frames only",
    status: "published",
    monetization: "mixed",
  },
  {
    id: "kw-trailer-leak",
    phrase: "gta vi trailer leak",
    cluster: "trailer",
    priority: "P1",
    locale: "en",
    targetSlugHint: "gta-6-trailer-3-what-we-know",
    relatedInternalPaths: [
      "/guides/gta-6-trailer-3-what-we-know",
      "/map",
      "/guides/gta-6-scam-watch",
    ],
    competitorNotes: "Label unverified claims clearly — AdSense safety",
    status: "published",
    monetization: "adsense",
  },
  // ── Map (core product) ─────────────────────────────────────────
  {
    id: "kw-interactive-map",
    phrase: "gta 6 interactive map",
    cluster: "map",
    priority: "P0",
    locale: "en",
    targetSlugHint: "gta-6-map-guide",
    relatedInternalPaths: ["/map", "/guides/gta-6-map-guide", "/locations"],
    status: "published",
    monetization: "map",
  },
  {
    id: "kw-vice-city-map",
    phrase: "gta 6 map vice city",
    cluster: "map",
    priority: "P0",
    locale: "en",
    targetSlugHint: "locations/vice-city",
    relatedInternalPaths: ["/map", "/locations/vice-city", "/guides/vice-city-locations"],
    status: "published",
    monetization: "map",
  },
  // ── Preorder (affiliate) ───────────────────────────────────────
  {
    id: "kw-preorder-ps5",
    phrase: "gta 6 preorder ps5",
    cluster: "preorder",
    priority: "P0",
    locale: "en",
    targetSlugHint: "gta-6-preorder-ps5-guide",
    relatedInternalPaths: ["/guides/gta-6-preorder-guide"],
    status: "published",
    monetization: "affiliate",
  },
  {
    id: "kw-collector",
    phrase: "gta 6 collector edition",
    cluster: "preorder",
    priority: "P1",
    locale: "en",
    targetSlugHint: "gta-6-collectors-edition",
    relatedInternalPaths: [
      "/guides/gta-6-collectors-edition",
      "/guides/gta-6-preorder-guide",
    ],
    status: "published",
    monetization: "affiliate",
  },
  {
    id: "kw-ultimate-edition",
    phrase: "gta 6 ultimate edition",
    cluster: "preorder",
    priority: "P0",
    locale: "en",
    targetSlugHint: "gta-6-ultimate-edition-vs-standard",
    relatedInternalPaths: [
      "/guides/gta-6-preorder-guide",
      "/guides/gta-6-collectors-edition",
    ],
    competitorNotes: "Compare Ultimate vs Standard/Collector once Rockstar lists editions — no fake ASINs",
    status: "published",
    monetization: "affiliate",
  },
  {
    id: "kw-preorder-price",
    phrase: "gta 6 preorder price",
    cluster: "preorder",
    priority: "P0",
    locale: "en",
    targetSlugHint: "gta-6-preorder-price-editions",
    relatedInternalPaths: [
      "/guides/gta-6-preorder-guide",
      "/guides/gta-6-collectors-edition",
    ],
    competitorNotes: "Price ranges only from official/store listings — never invent MSRP",
    status: "published",
    monetization: "affiliate",
  },
  {
    id: "kw-best-price-fr",
    phrase: "meilleur prix gta 6",
    cluster: "preorder",
    priority: "P0",
    locale: "fr",
    targetSlugHint: "gta-6-best-price",
    relatedInternalPaths: [
      "/guides/gta-6-best-price",
      "/guides/gta-6-preorder-guide",
      "/guides/gta-6-ultimate-edition-vs-standard",
    ],
    competitorNotes:
      "Multi-retailer comparator (Amazon/Fnac/Cdiscount/Carrefour vs official stores) — manual price survey, dated",
    status: "published",
    monetization: "affiliate",
  },
  {
    id: "kw-best-price-en",
    phrase: "gta 6 best price",
    cluster: "preorder",
    priority: "P1",
    locale: "en",
    targetSlugHint: "gta-6-best-price",
    relatedInternalPaths: [
      "/guides/gta-6-best-price",
      "/guides/gta-6-preorder-guide",
    ],
    competitorNotes: "Same comparator page — EN fallback copy, FR-market prices",
    status: "published",
    monetization: "affiliate",
  },
  // ── Locations ──────────────────────────────────────────────────
  {
    id: "kw-vice-locations",
    phrase: "vice city locations gta 6",
    cluster: "locations",
    priority: "P1",
    locale: "en",
    targetSlugHint: "vice-city-locations",
    relatedInternalPaths: ["/locations/vice-city", "/guides/vice-city-locations", "/map"],
    status: "published",
    monetization: "mixed",
  },
  {
    id: "kw-ocean-drive",
    phrase: "ocean drive gta 6",
    cluster: "locations",
    priority: "P1",
    locale: "en",
    targetSlugHint: "ocean-drive-gta-6",
    relatedInternalPaths: [
      "/guides/ocean-drive-gta-6",
      "/locations/ocean-drive",
      "/map",
    ],
    status: "published",
    monetization: "adsense",
  },
  // ── Collectibles ───────────────────────────────────────────────
  {
    id: "kw-hidden-packages",
    phrase: "gta 6 hidden packages",
    cluster: "collectibles",
    priority: "P1",
    locale: "en",
    targetSlugHint: "hidden-packages-gta-6",
    relatedInternalPaths: [
      "/collectibles",
      "/guides/hidden-packages-gta-6",
      "/map",
    ],
    status: "published",
    monetization: "mixed",
  },
  {
    id: "kw-collectibles-map",
    phrase: "gta 6 collectibles map",
    cluster: "collectibles",
    priority: "P0",
    locale: "en",
    targetSlugHint: "gta-6-collectibles-map",
    relatedInternalPaths: ["/collectibles", "/map"],
    status: "published",
    monetization: "map",
  },
  // ── Release ────────────────────────────────────────────────────
  {
    id: "kw-release-date",
    phrase: "gta 6 release date",
    cluster: "release",
    priority: "P0",
    locale: "en",
    targetSlugHint: "gta-6-release-date",
    relatedInternalPaths: ["/guides/gta-6-preorder-guide", "/map"],
    status: "published",
    monetization: "mixed",
  },
  {
    id: "kw-platforms",
    phrase: "gta 6 platforms",
    cluster: "release",
    priority: "P2",
    locale: "en",
    targetSlugHint: "gta-6-platforms-ps5-xbox",
    relatedInternalPaths: [
      "/guides/gta-6-platforms-ps5-xbox",
      "/guides/gta-6-ps5-vs-xbox",
      "/guides/gta-6-preorder-guide",
    ],
    status: "published",
    monetization: "affiliate",
  },
  // ── Characters / story (competitor gap vs Leonida Intel / wiki) ─
  {
    id: "kw-characters",
    phrase: "gta 6 characters",
    cluster: "characters",
    priority: "P0",
    locale: "en",
    targetSlugHint: "gta-6-characters-lucia-jason",
    relatedInternalPaths: [
      "/guides/gta-6-characters-lucia-jason",
      "/guides/gta-6-story",
      "/map",
    ],
    competitorNotes: "Lucia + Jason hub — MapGenie/wiki own this; map sites usually skip",
    status: "published",
    monetization: "mixed",
  },
  {
    id: "kw-lucia",
    phrase: "lucia caminos gta 6",
    cluster: "characters",
    priority: "P0",
    locale: "en",
    targetSlugHint: "gta-6-characters-lucia-jason",
    relatedInternalPaths: [
      "/guides/gta-6-characters-lucia-jason",
      "/guides/gta-6-story",
    ],
    status: "published",
    monetization: "adsense",
  },
  {
    id: "kw-jason",
    phrase: "jason duval gta 6",
    cluster: "characters",
    priority: "P0",
    locale: "en",
    targetSlugHint: "gta-6-characters-lucia-jason",
    relatedInternalPaths: [
      "/guides/gta-6-characters-lucia-jason",
      "/guides/gta-6-story",
    ],
    status: "published",
    monetization: "adsense",
  },
  {
    id: "kw-story",
    phrase: "gta 6 story",
    cluster: "story",
    priority: "P1",
    locale: "en",
    targetSlugHint: "gta-6-story",
    relatedInternalPaths: [
      "/guides/gta-6-story",
      "/guides/gta-6-characters-lucia-jason",
      "/guides/leonida-lore-overview",
    ],
    status: "published",
    monetization: "adsense",
  },
  // ── Map size / comparison ──────────────────────────────────────
  {
    id: "kw-map-size",
    phrase: "gta 6 map size",
    cluster: "map",
    priority: "P0",
    locale: "en",
    targetSlugHint: "gta-6-map-size",
    relatedInternalPaths: ["/guides/gta-6-map-size", "/map", "/guides/gta-6-map-guide"],
    competitorNotes: "Separate Rockstar quotes from 2–2.5× fan estimates",
    status: "published",
    monetization: "map",
  },
  {
    id: "kw-vs-gta5",
    phrase: "gta 6 vs gta 5 map",
    cluster: "map",
    priority: "P1",
    locale: "en",
    targetSlugHint: "gta-6-map-size",
    relatedInternalPaths: ["/guides/gta-6-map-size", "/map?game=gta5"],
    status: "published",
    monetization: "map",
  },
  // ── Vehicles ───────────────────────────────────────────────────
  {
    id: "kw-vehicles",
    phrase: "gta 6 vehicles",
    cluster: "vehicles",
    priority: "P1",
    locale: "en",
    targetSlugHint: "gta-6-vehicles",
    relatedInternalPaths: [
      "/guides/gta-6-vehicles",
      "/guides/gta-6-ultimate-edition-vs-standard",
      "/map",
    ],
    competitorNotes: "Trailer cars + Vintage Vice City Pack Stanier — no fake garage list",
    status: "published",
    monetization: "mixed",
  },
  {
    id: "kw-vintage-pack",
    phrase: "gta 6 vintage vice city pack",
    cluster: "preorder",
    priority: "P1",
    locale: "en",
    targetSlugHint: "gta-6-vehicles",
    relatedInternalPaths: [
      "/guides/gta-6-vehicles",
      "/guides/gta-6-preorder-guide",
      "/guides/gta-6-ultimate-edition-vs-standard",
    ],
    status: "published",
    monetization: "affiliate",
  },
  // ── Platforms / console compare / PC / safety ──────────────────
  {
    id: "kw-ps5-vs-xbox",
    phrase: "gta 6 ps5 vs xbox",
    cluster: "release",
    priority: "P0",
    locale: "en",
    targetSlugHint: "gta-6-ps5-vs-xbox",
    relatedInternalPaths: [
      "/guides/gta-6-ps5-vs-xbox",
      "/guides/gta-6-platforms-ps5-xbox",
      "/guides/best-setup-gta-6-ps5-xbox",
    ],
    competitorNotes: "Buyer-safe compare — no invented FPS charts",
    status: "published",
    monetization: "affiliate",
  },
  {
    id: "kw-pc-reqs",
    phrase: "gta 6 system requirements",
    cluster: "setup",
    priority: "P1",
    locale: "en",
    targetSlugHint: "gta-6-pc-requirements",
    relatedInternalPaths: [
      "/guides/gta-6-pc-requirements",
      "/guides/gta-6-platforms-ps5-xbox",
      "/guides/best-setup-gta-6-ps5-xbox",
    ],
    competitorNotes: "PC not announced — forecast labeled, no fake min spec table as fact",
    status: "published",
    monetization: "affiliate",
  },
  {
    id: "kw-pc-date",
    phrase: "gta 6 pc release date",
    cluster: "release",
    priority: "P1",
    locale: "en",
    targetSlugHint: "gta-6-pc-requirements",
    relatedInternalPaths: [
      "/guides/gta-6-pc-requirements",
      "/guides/gta-6-release-date",
      "/guides/gta-6-platforms-ps5-xbox",
    ],
    status: "published",
    monetization: "mixed",
  },
  {
    id: "kw-preload",
    phrase: "gta 6 preload",
    cluster: "release",
    priority: "P1",
    locale: "en",
    targetSlugHint: "gta-6-release-date",
    relatedInternalPaths: [
      "/guides/gta-6-release-date",
      "/guides/gta-6-preorder-guide",
    ],
    status: "published",
    monetization: "mixed",
  },
  {
    id: "kw-scam",
    phrase: "gta 6 fake download",
    cluster: "preorder",
    priority: "P1",
    locale: "en",
    targetSlugHint: "gta-6-scam-watch",
    relatedInternalPaths: [
      "/guides/gta-6-scam-watch",
      "/guides/gta-6-preorder-guide",
      "/guides/gta-6-pc-requirements",
    ],
    competitorNotes: "Trust page — APK/beta/key scams around live preorders",
    status: "published",
    monetization: "adsense",
  },
  // ── Setup / clip funnels (purchase + creator volume) ───────────
  {
    id: "kw-best-setup",
    phrase: "best setup for gta 6",
    cluster: "setup",
    priority: "P0",
    locale: "en",
    targetSlugHint: "best-setup-gta-6-ps5-xbox",
    relatedInternalPaths: [
      "/guides/best-setup-gta-6-ps5-xbox",
      "/guides/gta-6-preorder-guide",
      "/map",
    ],
    competitorNotes: "High EPC: console + headset + SSD bundle intent",
    status: "published",
    monetization: "affiliate",
  },
  {
    id: "kw-headset",
    phrase: "best headset for gta 6",
    cluster: "setup",
    priority: "P1",
    locale: "en",
    targetSlugHint: "best-setup-gta-6-ps5-xbox",
    relatedInternalPaths: ["/guides/best-setup-gta-6-ps5-xbox"],
    status: "published",
    monetization: "affiliate",
  },
  {
    id: "kw-map-clip",
    phrase: "gta 6 map tiktok",
    cluster: "clip",
    priority: "P1",
    locale: "en",
    targetSlugHint: "gta-6-map-clip-kit",
    relatedInternalPaths: [
      "/guides/gta-6-map-clip-kit",
      "/map",
      "/overlay",
      "/guides/gta-6-map-guide",
    ],
    competitorNotes: "Clip kit pages → share deep links → retention",
    status: "published",
    monetization: "map",
  },
];

export function getKeywordsByCluster(cluster: KeywordRecord["cluster"]) {
  return KEYWORDS.filter((k) => k.cluster === cluster);
}

export function getKeywordById(id: string) {
  return KEYWORDS.find((k) => k.id === id);
}

export function getP0Keywords() {
  return KEYWORDS.filter((k) => k.priority === "P0");
}
