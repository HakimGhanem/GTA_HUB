...
export function Cs2GuideContent({ locale }: Props) {
  ...
  return (
    <article className="prose prose-invert mt-8 max-w-none">
      <p className="text-lg leading-relaxed text-foreground/80">{copy.intro}</p>

      <h2 className="mt-10 text-2xl font-bold text-foreground">{copy.whatTitle}</h2>
      <p className="leading-relaxed text-foreground/80">{copy.whatBody}</p>
      <ul className="mt-4 list-disc space-y-1 pl-5 text-foreground/70">
      ...
      <div className="not-prose my-8 flex flex-wrap gap-3">
        <a
          ...
          className="inline-flex rounded-full bg-[#1b2838] px-5 py-2.5 text-sm font-semibold text-foreground transition-colors hover:bg-[#2a475e]"
        >
          {copy.ctaSteam}
        </a>
        <Link
      ...
      <h2 className="mt-10 text-2xl font-bold text-foreground">{copy.map6Title}</h2>
      <p className="leading-relaxed text-foreground/80">{copy.map6Body}</p>

      <h2 className="mt-10 text-2xl font-bold text-foreground">{copy.regionsTitle}</h2>
      <div className="not-prose mt-4 grid gap-3 sm:grid-cols-2">
        {REGIONAL_LOCATION_SLUGS.map((slug) => (
          <Link
            key={slug}
            href={`/locations/${slug}`}
            className="rounded-xl border border-foreground/10 bg-foreground/5 px-4 py-3 text-sm font-medium text-foreground transition-colors hover:border-pink-400/40"
          >
            {REGION_NAMES[slug]}
      ...
      <p className="leading-relaxed text-foreground/60">{copy.disclaimerBody}</p>
